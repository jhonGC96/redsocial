...
module.exports = class Datos {
    ...
    static async listar (data){
        ...
        try {
            let resultado = await sequelize.query(`SELECT * FROM Usuario WHERE id = ?`,
    ...
    static async alta (data){
        ...
        try {
            //Uso de objetos
            let resultado = await sequelize.query(`INSERT INTO Usuario(nombre_usr, apellido_usr, email_usr, edad_usr, contrasena, id_pais1, perfillinkedin) VALUES (?,?,?,?,?,?,?)`,
    ...
    static async existenciaDeUsuario (data){
        ...
        try {
            let resultado = await sequelize.query(`SELECT * FROM Usuario WHERE email_usr = ?`,
            {replacements : usuarioUpdate, type : sequelize.QueryTypes.SELECT})
            if (resultado[0] === undefined) {
                return false
            } else {
